...
std::string GetGameSateJsonBody(const app::game_state::Result& game_state){
    ...
    for(const auto& dog : game_state.dogs){
        ...
        player[GameSateFields::SCORE] = dog.score; 
...
std::string GetRecordsJsonBody(const model::Records& records) {
    boost::json::array res; 
    res.reserve(records.size());
    for (const auto& record: records) {
        boost::json::object obj;
        obj[RecordsFields::NAME] = record.name;
        obj[RecordsFields::SCORE] = record.score;
        obj[RecordsFields::PLAYTIME] = record.play_time;
        res.emplace_back(std::move(obj));
    }
    return serialize(res);
}
