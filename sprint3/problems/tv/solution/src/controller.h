...
private:
    ...
    [[nodiscard]] bool SelectChannel(std::istream& input, std::ostream& output) const {
        using namespace std::literals;
        assert(input);
        // убедимся, что тв включен
        if(!EnsureTvIsTurnedOn(output)) {
            return false;
        }
        // считываем номер канала
        std::string data; 
        input >> data; 
        // если нет канали или номер канала дробное число, то выводим сообщение об ошибке    
    ...
    [[nodiscard]] bool SelectPreviousChannel(std::istream& input, std::ostream& output) const {
        ...
        // если тв выключен
        if (EnsureTvIsTurnedOn(output) &&
            EnsureNoArgsInInput(SELECT_PREVIOUS_CHANNEL_COMMAND, input, output)) {
    ...
    [[nodiscard]] bool EnsureTvIsTurnedOn(std::ostream& output) const {
        using namespace std::literals;
        if(!tv_.IsTurnedOn()) {
            output << "TV is turned off"sv << std::endl;
            return false;
