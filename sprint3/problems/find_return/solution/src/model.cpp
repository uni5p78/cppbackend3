...
void Dog::SetNewPosOnRoad(const Map& map, const std::chrono::milliseconds time_delta){
...
const Dog::LootIdList& Dog::GetBag() {
    return bag;
}    
...
void CalcLootCollection(const ItemGathererContaner::Gatherers& gatherers, const model::GameSession& session){
    ...
    for (const auto& event : gathering_events) {
        ...
        int bag_capacity = session.GetMap().GetBagCapacity();
        auto loots = session.GetLoots();
        auto bag = dog->GetBag();
...
void Loots::AddLoot(Loot loot) {
    // loot.active = true;
    // loot.status_ = LootStatus::ON_MAP;
    bool insert = false;
    size_t id = 0;
...
void Loots::SetLootStatusById(size_t id, LootStatus status) {
    ...
    // if (loot_status_ == LootStatus::ON_MAP) {
    //     loots_on_map_count_--;        
    // } else if (status  == LootStatus::ON_MAP) {
    //     loots_on_map_count_++;
    // }    
